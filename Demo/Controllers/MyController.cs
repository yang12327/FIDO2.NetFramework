...
namespace Fido2NetFrameworkDemo.Controllers
{
    public class MyController : Controller
    {
        ...
        public MyController()
        {
            int TimestampDriftTolerance;
            if (!int.TryParse(ConfigurationManager.AppSettings["TimestampDriftTolerance"], out TimestampDriftTolerance))
        ...
        public ActionResult MakeCredentialOptions(string username,
                                                string displayName,
                                                string attType,
                                                string authType,
                                                ...
                                                string userVerification)
        {
            try
            {
                ...
                var options = _fido2.RequestNewCredential(user, existingKeys, authenticatorSelection, attType.ToEnum<AttestationConveyancePreference>(), exts).ToJson();

                // 4. Temporarily store options, session/in-memory cache/redis/db
                HttpContext.Session["fido2.attestationOptions"] = options;
        ...
        public async Task<ActionResult> MakeCredential(AuthenticatorAttestationRawResponse attestationResponse)
        {
            try
            {
                // 1. get the options we sent the client
                attestationResponse.Type = PublicKeyCredentialType.PublicKey;
                var jsonOptions = HttpContext.Session["fido2.attestationOptions"].ToString();
                ...
                return Content(JsonConvert.SerializeObject(success), "application/json");
            }
            catch (Exception e)
            {
        ...
        public ActionResult AssertionOptionsPost(string username, string userVerification)
        {
            try
            {
                ...
                // 4. Temporarily store options, session/in-memory cache/redis/db
                HttpContext.Session["fido2.assertionOptions"] = options;
        ...
        [HttpPost]
        [Route("makeAssertion")]
        public async Task<ActionResult> MakeAssertion(AuthenticatorAssertionRawResponse clientResponse)
        {
            try
            {
                // 1. Get the assertion options we sent the client
                clientResponse.Type = PublicKeyCredentialType.PublicKey;
                var jsonOptions = HttpContext.Session["fido2.assertionOptions"].ToString();
